...
const BaristaOrderCard = ({ order, refetch }) => {
  ...
  // Visual timers logic
  let headerColor = "bg-[#1f1f1f]"; // default dark
  let timeTextColor = "text-[#ababab]";
  ...
  return (
    <div className={`w-full bg-[#262626] rounded-2xl overflow-hidden shadow-lg border-2 flex flex-col h-full transform transition-all ${pulseEffect ? "border-red-500/50 shadow-[0_0_15px_rgba(239,68,68,0.2)]" : "border-[#333]"}`}>
      
      {/* Dynamic Header */}
      <div className={`${headerColor} p-4 flex justify-between items-center border-b border-[#333] transition-colors`}>
        <div>
          <div className="flex items-center gap-2">
            <h1 className="text-white font-black text-2xl">Mesa {order.table?.tableNo || "?"}</h1>
            {isHistory && <span className="bg-[#142636] text-blue-400 text-[10px] px-2 py-0.5 rounded-full font-bold">HISTORIAL</span>}
          </div>
          <p className="text-[#ababab] text-sm truncate max-w-[120px]" title={order.customerDetails?.name}>
      ...
      <div className="p-4 flex-1 overflow-y-auto min-h-[150px]">
        <h2 className="text-[#ababab] text-[11px] font-bold mb-3 border-b border-[#333] pb-2 uppercase tracking-widest text-left">
          Cantidades
        </h2>
            ...
            return (
              ...
              >
                 <div className="flex gap-4 items-center w-full">
                   ...
                   <span className={`font-black rounded px-3 py-1 text-lg shadow-inner ${isChecked ? 'bg-green-900/30 text-green-500' : 'bg-[#333] text-white'}`}>
                     {item.quantity}x
                   </span>
                   
                   <div className="flex flex-col justify-center flex-1">
                      <span className={`text-lg font-medium leading-tight transition-colors ${isChecked ? 'text-[#888] line-through' : 'text-white'}`}>
                         {item.name}
                      </span>
                   </div>
                 </div>
