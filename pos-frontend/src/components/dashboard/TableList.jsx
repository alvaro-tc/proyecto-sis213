...
const TableList = ({ onEdit, onAdd }) => {
  ...
  if (isLoading) return <div className="text-white p-6 justify-center flex">Cargando mesas...</div>;

  return (
    <div className="container mx-auto py-2 px-6 md:px-4">
      <div className="flex justify-between items-center mb-4">
        <h2 className="text-[#f5f5f5] text-xl font-semibold">Listado de Mesas</h2>
      ...
      <div className="bg-[#1a1a1a] rounded-lg overflow-hidden">
        <table className="w-full text-left text-[#ababab]">
          ...
          <tbody>
            {tables.map((t) => (
              <tr key={t._id} className="border-b border-[#262626] hover:bg-[#262626]">
                <td className="p-4 font-semibold text-white">Mesa {t.tableNo}</td>
